...
{
  internal class UpdateYurPostsFrom1C : SPJobDefinition
  {
    public UpdateYurPostsFrom1C()
    {
    }
    ...
    public UpdateYurPostsFrom1C(string jobName, SPWebApplication webApplication)
      : base(jobName, webApplication, (SPServer) null, SPJobLockType.ContentDatabase)
    {
      this.Title = "Update Yur Posts From 1C";
    }
          ...
          for (int index = 0; index < this.WebApplication.Sites.Count; ++index)
          {
            using (SPSite site = this.WebApplication.Sites[index])
            {
              foreach (SPWeb allWeb in (SPBaseCollection) site.AllWebs)
              {
                ...
                try
                {
                  spList = allWeb.GetList("Lists/YurPostsList");
                }
                catch (Exception ex)
                {
                }
                if (spList != null)
                  BusinessLogic.Fill1CList(site.Url.ToString(), allWeb.ServerRelativeUrl.ToString(), "Lists/YurPostsList", "ДолжностиОрганизаций");
              }
            }
          }
        }));
